...
if page == 'Match Simulator':
    ...
    with col_mc:
        ...
        if not mc_match.empty:
            ...
            # Probability bars from prob_df
            home_prob_row = prob_df[prob_df['team'] == home_team]
            away_prob_row = prob_df[prob_df['team'] == away_team]

            if not home_prob_row.empty and not away_prob_row.empty:
                st.markdown('#### Win probabilities (from simulations)')
                home_adv = home_prob_row.iloc[0]['advancement%']
                away_adv = away_prob_row.iloc[0]['advancement%']
                st.markdown(f'**{home_team}** advancement: `{home_adv}%`')
                st.progress(int(home_adv))
                st.markdown(f'**{away_team}** advancement: `{away_adv}%`')
                st.progress(int(away_adv))
        else:
